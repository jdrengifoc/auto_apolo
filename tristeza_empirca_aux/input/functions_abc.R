...
run_ABC_pipeline <- function(app_name, data_path, formula_list, prior_list, model_type_list,
  ...
  results_AR   <- list()   # For Accept/Reject ABC
  results_MCMC <- list()   # For MCMC ABC
  ...
  # par(mfrow = c(3, 1))
  # plot(PostAR, main = paste("Accept-Reject ABC (AR) -", app_name))
  # plot(PostMCMC, main = paste("MCMC ABC -", app_name))
